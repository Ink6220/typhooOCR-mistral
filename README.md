ได้เลยครับ นี่คือ `README.txt` เวอร์ชันสมบูรณ์ที่รวมทุกส่วนของโปรเจกต์ พร้อมตัวอย่างการใช้งานโดยละเอียดครับ
...
สคริปต์ Python นี้ใช้สำหรับประมวลผลไฟล์เอกสาร (รูปภาพ หรือ PDF) โดยการดึงโครงสร้างและเนื้อหาออกมา จากนั้นส่งไปให้ **OpenTyphoon API** เพื่อทำการ OCR และวิเคราะห์โครงสร้างเอกสาร ผลลัพธ์ที่ได้จะถูกบันทึกเป็นไฟล์ข้อความ (`.txt`)

โค้ดส่วนหลักที่ใช้ในการประมวลผลไฟล์ PDF ถูกดัดแปลงมาจากโปรเจกต์ [olmocr](https://github.com/allenai/olmocr) ซึ่งอยู่ภายใต้ใบอนุญาต Apache 2.0

-----

## ✨ คุณสมบัติ (Features)

  * **รองรับหลายไฟล์**: สามารถประมวลผลได้ทั้งไฟล์รูปภาพ (`.jpg`, `.png`, etc.) และไฟล์ PDF
  * **แปลงไฟล์อัตโนมัติ**: หากไฟล์ที่นำเข้าเป็นรูปภาพ สคริปต์จะแปลงเป็น PDF ชั่วคราวเพื่อประมวลผล
  * **เรียกใช้ Typhoon API**: ส่งข้อมูลรูปภาพและโครงสร้างที่สกัดได้ไปยัง `typhoon-ocr-preview` เพื่อการอ่านข้อความที่แม่นยำ
  * **จัดการข้อมูลสำคัญอย่างปลอดภัย**: ใช้ไฟล์ `.env` ในการจัดเก็บ API Key เพื่อไม่ให้ข้อมูลรั่วไหลในโค้ด
  * **บันทึกผลลัพธ์**: จัดเก็บผลลัพธ์ที่ได้จาก API เป็นไฟล์ `.txt` อย่างเป็นระเบียบในโฟลเดอร์ `ocr_results`
...
1.  **Python 3.8+**
2.  **Poppler-utils**: เป็นเครื่องมือสำหรับจัดการไฟล์ PDF ซึ่งสคริปต์นี้ใช้ในการแปลง PDF เป็นรูปภาพ (`pdftoppm`) และดึงข้อมูล (`pdfinfo`)
      ...
      * **บน Debian/Ubuntu:**
        ...
        ```
      * **บน Windows:** ดาวน์โหลดและติดตั้งจาก [ที่นี่](https://github.com/oschwartz10612/poppler-windows/releases/) และทำการเพิ่ม Path ของโฟลเดอร์ `bin` ไปยัง Environment Variables

-----

## ⚙️ การติดตั้งและตั้งค่า (Installation & Setup)

1.  **Clone หรือดาวน์โหลดสคริปต์:**
    นำไฟล์ `typhoon_tranformers_app_API.py` มาไว้ในโฟลเดอร์โปรเจกต์ของคุณ
...
2.  **ติดตั้ง Dependencies ของ Python:**
    ...
    *(คุณจะต้องสร้างไฟล์ `requirements.txt` ที่มีเนื้อหาดังนี้):*
...
3.  **ตั้งค่า API Key:**
      ...
      * สร้างไฟล์ใหม่ในโฟลเดอร์เดียวกันชื่อว่า `.env`
      * เปิดไฟล์ `.env` แล้วเพิ่ม API Key ของคุณลงไป:
        ```
        TYPHOON_API_KEY="YOUR_TYPHOON_API_KEY_GOES_HERE"
        ```
      * **อย่าลืม\!** นำไฟล์ `.env` ไปใส่ใน `.gitignore` เพื่อป้องกันไม่ให้ Key ของคุณหลุดขึ้นไปยัง Git Repository
...
1.  **แก้ไขไฟล์สคริปต์:**
    เปิดไฟล์ `typhoon_tranformers_app_API.py` และแก้ไขค่าในส่วนของ `if __name__ == "__main__":`

    ```python
    # ระบุชื่อไฟล์รูปภาพหรือ PDF ของคุณที่ต้องการทดสอบ
    input_file = "path/to/your/document.pdf"
    page_to_process = 1
    task = "structure" # สามารถเลือก 'default' หรือ 'structure'
    ```
...
2.  **รันสคริปต์:**
    ...
    ```bash
    python typhoon_tranformers_app_API.py
    ```

สคริปต์จะเริ่มทำงาน และเมื่อประมวลผลเสร็จสิ้น ผลลัพธ์จะถูกแสดงใน Console และบันทึกเป็นไฟล์ `.txt` ในโฟลเดอร์ `ocr_results`