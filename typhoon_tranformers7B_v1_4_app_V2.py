...
# [ใหม่] ฟังก์ชันสำหรับทำความสะอาดผลลัพธ์จากโมเดลโดยเฉพาะ
def clean_model_response(response: str) -> str:
    """
    ทำความสะอาดผลลัพธ์ที่ได้จากโมเดล เช่น ลบ JSON ที่ไม่ต้องการออก
    """
    # ลบ JSON object ที่ขึ้นต้นด้วย "primary_language" ซึ่งเป็น noise จากโมเดล
    cleaned_response = re.sub(r'\{\s*"primary_language":.*?\}', '', response, flags=re.DOTALL)
...
# [ปรับปรุง] แก้ไขฟังก์ชันให้จัดการย่อหน้า (\n\n) ได้ถูกต้อง
def convert_results_to_html(results: list) -> str:
    ...
    for elem in results:
        ...
        else: # para, cap, foot, fnote, list, etc.
            # แยกข้อความเป็นย่อหน้าด้วย \n\n หรือ \n \n
            paragraphs = re.split(r'\n\s*\n', text)
            for para in paragraphs:
                if para.strip():
                    # ในแต่ละย่อหน้า ให้แปลง \n เป็น <br>
                    line_break_para = para.strip().replace('\n', '<br>')
                    html_parts.append(f'<p>{line_break_para}</p>')
...
# [ปรับปรุง] คลีนโค้ดและเพิ่มการเรียกใช้ฟังก์ชันทำความสะอาด
    ...
    try:
        ...
        for i, (bbox, initial_label) in enumerate(layout_data):
            ...
            if initial_label == 'fig':
                ...
                # ทำความสะอาดผลลัพธ์ที่ได้จากการตรวจสอบ
                cleaned_type_response = clean_model_response(type_response)
                logger.info(f"Element {i} classification response: '{cleaned_type_response}'")

                # --- ขั้นตอนที่ 2: แปลงข้อมูลตามประเภทที่ตรวจสอบได้ ---
                if "TABLE" in cleaned_type_response.upper():
                    final_label = 'tab'
                    ...
                    html_prompt = (
                        ...
                        "**คำเตือน:** ห้ามตอบกลับมาในรูปแบบ JSON `{\"primary_language\":...}` โดยเด็ดขาด"
                    )
                    raw_html = process_with_thai_transformer_model(html_prompt, pil_crop)
                    final_text = clean_model_response(raw_html) # ทำความสะอาดผลลัพธ์ HTML ด้วย
                else: 
                    final_label = 'para'
                    ...
                    final_text = clean_model_response(raw_text) # ทำความสะอาดผลลัพธ์ข้อความด้วย
